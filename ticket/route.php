...
$errorMsgReg=''; $routeErr = ''; $amountErr= '';
...
if (isset($_POST['create'])) {
 ...
 if ($_SERVER["REQUEST_METHOD"] == "POST") {
            if (empty($_POST["route"])) {
               echo $routeErr = "<span style=\"color:red;\">Route is required</span>";
            }else {
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
            if (empty($_POST["amount"])) {
             echo $amountErr = "<span style=\"color:red;\">Amount cannot be Empty</span>";
            }else {
...
$remark= $_POST['remark'] = 'created by '.' '.$adminname;
  ...
  # code...
    ...
    $rid=$userClass->addRoute($route, $amount, $datecreated, $remark);
    if($rid)
    {
      
      ?>
      <script>
        alert('New Route Added Successfully, Click OK to continue...');
...
<head>
  ...
  <title>NIS Transport | Routes</title>
        ...
        <div class="row">
          ...
          <div class="card card-info">
              <div class="card-header">
                <h3 class="card-title">List of Registered Routes</h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
...
$result = $db->prepare("SELECT * FROM route");
            ...
            ?>
                  ...
                  <tr>
                    <td>NIS-R10-<?php echo $row['r_id']; ?></td>
                    <td><?php echo $row['route']; ?></td>
                    <td><?php echo $amt = number_format($row['amount']); ?></td>
