      ...
      <div class="container-fluid">
        <div class="row mb-1">
          <div class="col-sm-12 text-center">
            ...
            <!--<button type="button" class=" btn btn-outline-info" data-toggle="modal" data-target="#modal-dispatch"><i class="fas fa-cart-plus"></i> <h5>Add New Stock</h5></button>          
          -->
...
$db = getDB();
$result = $db->prepare("SELECT * FROM sale WHERE status = 'completed' AND staff='$adminname' ORDER BY datecreated DESC");
$result->execute();

for ($i = 1; $row = $result->fetch(); $i++) {
?>
    <tr>
        <td><?php echo htmlspecialchars($row['transcode']); ?></td>
        <td><?php echo htmlspecialchars($row['pname']); ?></td>
        <td><?php echo htmlspecialchars($row['qty']); ?></td>
        <td><?php echo htmlspecialchars($row['saleprice']); ?></td>
